...
public class MathController {
    ...
    public String add(@PathVariable int number1, @PathVariable int number2) {
        return number1 + " added to " + number2 + " equals: " + (number1+number2);
    }

    @GetMapping("/subtract/{number1}/{filler}/{number2}")
    @ResponseBody
    ...
    @GetMapping("/multiply/{number1}/{filler}/{number2}")
    @ResponseBody
    ...
    @GetMapping("/divide/{number1}/{filler}/{number2}")
    @ResponseBody
